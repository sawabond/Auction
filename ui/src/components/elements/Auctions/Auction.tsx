...
function Auction({ auction }: any) {
  ...
  const formattedAuctionStartTime: string = auctionStartTime.toLocaleString(
    LOCALE,
    OPTIONS
  );
  const auctionEndTime: Date = new Date(auction.endTime);
  const formattedAuctionEndTime: string = auctionEndTime.toLocaleString(
    LOCALE,
    OPTIONS
  );
  ...
  return (
    <ListItem
      key={auction.id}
      className="hover:bg-gray-100 grid grid-cols-3 gap-4"
    >
      <div>
