...
function AuctionItem({
  ...
  return (
    <div
      className={`flex justify-around items-center m-5 border p-4 rounded-lg ${isSold ? (isCurrentUserTheBuyer ? 'bg-yellow-400' : 'bg-red-200') : isCurrentlySelling ? 'bg-green-200' : 'bg-gray-200'} space-x-4`}
    >
          ...
          {item.photos.map((photo, index) => (
            ...
            >
              <img
                src={photo.photoUrl}
                alt={`Photo ${index + 1}`}
                className="w-full h-auto object-cover rounded-lg border border-gray-300" // Image fills width, maintains aspect ratio
              />
            </div>
          ))}
        </Slider>
