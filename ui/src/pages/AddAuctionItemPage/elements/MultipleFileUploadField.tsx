...
function MultipleFileUploadField({ name, onFilesChange }: { name: string; onFilesChange: Function }) {
      ...
      {files.map(fileWrapper => (
        <Grid item key={fileWrapper.id}>
          {fileWrapper.errors.length > 0 ? (
            ...
            <SingleFileUploadWithProgress file={fileWrapper.file} onUpload={handleUpload} onDelete={() => handleDelete(fileWrapper.file)} />
          )}
        </Grid>
      ))}
    </React.Fragment>
